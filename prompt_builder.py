...
class PromptBuilder:
    ...
    def build_prompt(
        ...
        try:
            ...
            filtered_agent_qa = [
                item
                for item in agent_qa
                if item.get("a") and not PromptBuilder.is_error_text(item.get("a"))
            ]
            if filtered_agent_qa:
                formatted = PromptBuilder.format_memories(filtered_agent_qa, limit=3)
                prompt = (
                    "[Agent recent context: " + " | ".join(formatted) + "]\n\n" + prompt
                )
            ...
            if include_group:
                ...
                filtered_group_qa = [
                    item
                    for item in group_qa
                    if item.get("a") and not PromptBuilder.is_error_text(item.get("a"))
                ]
                if filtered_group_qa:
                    gformatted = PromptBuilder.format_memories(
                        filtered_group_qa, limit=3
                    )
                    prompt = (
