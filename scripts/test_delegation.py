...
def main():
    ...
    if not orch.agents:
        logging.getLogger(__name__).warning(
            "No agents configured. Please populate agents_config.json and try again."
        )
        return
    ...
    if not orch.memory_db:
        logging.getLogger(__name__).warning(
            "Warning: MemoryDB not initialized. Delegation test will still run but won't persist."
        )
    ...
    if orch.memory_db:
        ...
        try:
            ...
            for r in group:
                logging.getLogger(__name__).info(
                    "agent=%s | question=%s | answer=%s | conv_id=%s | ts=%s",
                    r.get("agent_name"),
                    r.get("question"),
                    r.get("answer"),
                    r.get("conv_id"),
            ...
            logging.getLogger(__name__).exception(
                "Error while inspecting MemoryDB: %s", e
            )
