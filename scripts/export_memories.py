...
def main():
    ...
    group.add_argument(
        "--group", action="store_true", help="Export group memories (key=__group__)"
    )
    ...
    parser.add_argument(
        ...
        choices=["csv", "json"],
        default=None,
        help="Output format (inferred from file ext if omitted)",
    )
    parser.add_argument(
        "--limit", type=int, default=1000, help="Max number of rows to export"
    )
    ...
    logging.getLogger(__name__).info(
        f"Exporting {'group' if args.group else 'agent ' + (args.agent or '')} memories to {out_path} (limit={args.limit})"
    )
    ...
    logging.getLogger(__name__).info(
        f"Export complete: wrote {len(normalized)} rows to {out_path}"
    )
