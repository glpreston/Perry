...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Export conversation rows by conv_id from agent_memory"
    )
    parser.add_argument(
        "--list", action="store_true", help="List recent conversation ids"
    )
    ...
    parser.add_argument(
        "--limit", type=int, default=20, help="Limit for listing conv ids"
    )
    ...
    def main():
        ...
        if args.list:
            rows = db._try_execute(
                "SELECT conv_id, COUNT(*) as cnt, MAX(timestamp) as last_ts FROM agent_memory GROUP BY conv_id ORDER BY last_ts DESC",
                (),
                fetch=True,
            )
            if not rows:
            ...
            for conv_id, cnt, last_ts in rows:
                logging.getLogger(__name__).info(
                    f"{conv_id}  — {cnt} rows — last: {last_ts}"
                )
            return 0
        ...
        out_path = (
            Path(args.out) if args.out else out_dir / f"conversation_{conv_id}.json"
        )
        rows = db._try_execute(
            "SELECT agent_name, question, answer, conv_id, timestamp FROM agent_memory WHERE conv_id=%s ORDER BY timestamp ASC",
            (conv_id,),
            fetch=True,
        )
        if not rows:
