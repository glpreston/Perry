...
def test_delegation_primary_rephrase_and_moderator(monkeypatch):
    ...
    def test_delegation_primary_rephrase_and_moderator(monkeypatch):
        ...
        def fake_post(url, json, timeout=30):
            ...
            if 'You are Perry' in sys and calls['n'] == 1:
                class R:
                    ...
                    def json(self):
                        return {'response': "Perry: I think PI is approximately 3.14159"}
            ...
            if 'You are Perry' in sys and calls['n'] > 1:
                class R:
                    ...
                    def json(self):
                        return {'response': "Perry: Quoting Netty - PI^10 = 93648.04747608302"}
            ...
            if 'Moderator' in sys:
                class R:
                    ...
                    def json(self):
                        return {'response': "Moderator: Netty's numeric reply is the most direct and correct. Recommended: 93648.04747608302"}
        ...
        assert 'Netty' in replies or 'PI^10' in replies['Perry'] or '93648' in replies['Perry']
        # Moderator should provide the recommendation
        assert 'Moderator' in replies
        assert 'Recommended' in replies['Moderator'] or 'recommended' in replies['Moderator']